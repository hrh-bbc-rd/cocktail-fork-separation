...
class MRX(torch.nn.Module):
    ...
    *THE COCKTAIL FORK PROBLEM: THREE-STEM AUDIO SEPARATION FOR REAL-WORLD SOUNDTRACKS*
    ...
    Args:
        n_sources (int, optional): The number of sources to separate. window_lengths
        (List[int], optional): List of multiple STFT window sizes.
                                              Default is None, which uses [1024, 2048,
                                              8192]
        n_hop (int, optional): STFT hop-size. Constant across resolutions. n_hidden
        (int, optional): The number of hidden units throughout the network.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Perform source separation. Generate audio source waveforms. Args:
...
class _EncoderBlock(torch.nn.Module):
    def __init__(self, num_inputs: int, num_hidden: int) -> None:
        """
        CrossNet encoder originally proposed in Sawata et al. "All for one and one for
        all: Improving music separation by bridging networks," ICASSP 2021.

        Our implementation differs in two main ways: (1) we use a BatchNorm layer on
        input instead of precomputing mean/std, and (2) we encode each channel
        independently to handle variable number of channels at inference time.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor):  3D Tensor with shape [batch, channels, freqs, frames]
                               or 2D Tensor of shape [channels, freqs, frames]. Complex
                               STFT.
        Returns:
...
class _DecoderBlock(torch.nn.Module):
    """
    CrossNet decode block originally proposed in Sawata et al. "All for one and one for
    all: Improving music separation by bridging networks," ICASSP 2021. The difference
    in our implementation is that each channel is decoded independently to handle
    variable number of channels at inference time.
    """
...
class _CrossNet(torch.nn.Module):
    """
    Parallel LSTM branches with averaging layers. Architecture was originally proposed
    in Sawata et al. "All for one and one for all: Improving music separation by
    bridging networks," ICASSP 2021. In our multi-resolution extension each branch takes
    a different input.
    """
