...
def mixture_consistency(
    ...
    """
    Postprocessing for adding residual between mixture and estimated sources back to
    estimated sources.

    :param mixture (torch.tensor): audio mixture signal
    :param estimated_sources (torch.tensor): estimated separated source signals with
        added source dimension
    :param source_dim (int): dimension of sources in estimated source tensor
    :param source_weights (list): the weights for each source. Length must match the
        source_dim of estimated sources
    :return:
    """
...
def dnr_consistency(
    ...
    :param mixture (torch.tensor): 3D Tensor with shape [batch, channels, samples] or 2D
                                   Tensor of shape [channels, samples]
    :param estimated_sources (torch.tensor): 4D Tensor with shape [batch, num_sources,
                                             channels, samples] or 3D Tensor of shape
                                             [num_sources, channels, samples]
    :param mode (str): choices=["all", "pass", "music_sfx"], Whether to add the residual
                       to estimates, 'pass' doesn't add residual, 'all' splits residual
                       among all sources, 'music_sfx' splits residual among music and
                       sfx sources . (default: pass)"
