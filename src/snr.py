...
def snr_loss(
    ...
    :param estimates (torch.Tensor): estimated source signals, tensor of shape [...,
    n_samples, ....] :param targets (torch.Tensor): ground truth signals, tensor of
    shape [...., n_samples, ....] :param dim (int): time (sample) dimension :param
    scale_invariant (bool): use SI-SNR when true, regular SNR when false :return
    (torch.Tensor): estimated [SI-]SNR with one value for each non-sample dimension
    """
    if scale_invariant:
    ...
    si_snr_array = _l2_square(s_target, dim=dim) / (
        _l2_square(e_noise, dim=dim) + EPSILON
    )
...
def _l2_square(
    arr: torch.Tensor, dim: Optional[int] = None, keepdim: Optional[bool] = False
) -> torch.Tensor:
